...
### Two poster/intern pipelines feeding these

1. **Intern projects.** Interns submit their projects for presentation. A selection committee
   picks **a few for the Intern Showcase lightning talks** (lunch, plenary); the **remaining
   accepted projects become poster presentations.** One submission pool, two outcomes.
2. **Student poster CFP.** A **separate Call for Proposals for posters from area students** feeds
   the poster display alongside the intern posters.

> **Note for the CFP / logistics:** both pipelines need submission deadlines, a selection/review
> step, poster size/format specs, easel or board counts, and assigned display locations. The
> intern CFP must flag that submitting means *either* a lightning talk *or* a poster.
...
when the second lecture block ends, so the grid stays in sync (2 workshops vs. 4 lectures per
other room). Workshops run **continuous** through the lecture-room break, with a facilitator-called
informal in-room break.

---

## Resolved layout decisions

1. **Ballroom seating:** **full banquet rounds** all day (max capacity; no seating flip).
...
- **Panels as an anchor:** schedule the ballroom's larger-draw **panels** opposite the workshops
  so the two "marquee" formats don't compete for the same audience.
